...
def create_new_node(path):
    try:
        ...
        for line in user.readlines():
            dic = json.loads(line)
            if '发动机数量' in dic.keys():
                new_data = dic['发动机数量']
                if new_data != '':
                    if graph.nodes.match('发动机数量', name=new_data).first() is None:
                        new_node = Node('military','发动机数量',new_data,
                                              name = new_data
                                              )
                        ...
                        node_relationship(
                            graph.nodes.match('名称', name = dic['名称']).first(),
                            '发动机数量',
                            graph.nodes.match('发动机数量', name = new_data).first()
                        )
                        print('============= ' + str(new_data) + " =============节点创建成功")
                else:
                    print('----------------------------------')
